...
void InitCirLinkList(CirLinkList *tail)
{
	...
	printf("%d\n", __LINE__);
	// 初始化头结点信息
	if((*tail = (CirLinkListNode *)malloc(sizeof(CirLinkListNode))) == NULL)		// 开辟单链表尾结点（即头结点）的空间
	{	// 开辟失败
...
//	CirLinkListNode *pNode = (*tail)->m_(*tail)->m_next;			// 当循环链表只设尾指针时采用此代码
...
//	return (*tail)->m_(*tail)->m_next->m_next;			// 只设尾指针的时候使用此代码
}
...
{
	return (*tail)->m_next;
//	return (*tail)->m_(*tail)->m_next;			// 只设尾指针的时候使用此代码
}
