...
/*=========================【PUBLIC：公共接口】=========================*/
  ...
  // 类的特征成员函数
    ...
    int FindElem(ElemType &elem);              // 寻找当前元素值在链表中的位置

    bool GetElem(int position, ElemType &elem);              // 获取第position个结点的数据

    ElemType GetElem(int position);                         // 获取第position个结点的数据
    ...
    ElemType Delete(int position);              // 删除链表第position个结点的数据, 并将此数据返回给elem
...
std::cout <<"GetElemPtr..." <<std::endl;
   ...
   // system("pause");
   std::cout <<this->m_curPtr->m_data <<std::endl;
   // system("pause");
...
/**类的特征成员函数【实现】**************************************************
    ...
    void Traverse(void(*visit)(const ElemType &)) const;//遍历线性表
    bool FindElem(ElemType &elem);              // 寻找当前元素值在链表中的位置
    bool GetElem(int psition, ElemType &elem);//获取第position个结点的数据
    ElemTyep GetElem(int position);                         // 获取第position个结点的数据
    bool SetElem(int position, const ElemType &elem);// 设置第position个结点的数据域为elem
    bool Delete(int position, ElemType &elem);// 删除链表第position个结点的数据, 并将此数据返回给elem
    bool Insert(int position, const ElemType &elem);// 将数据elem插入到第position个结点
...
// 函数名称：bool FindElem(ElemType &elem);              //
// 操作如果：寻找当前元素值在链表中的位置
template <class ElemType>
int LinkList<ElemType>::FindElem(ElemType &elem)              // 寻找当前元素值在链表中的位置
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Traverse..." <<std::endl;
    ...
    {
        this->m_curPosition = 0;        // 将当前元素位置标识标识到第一个元素
    }
    for(Node<ElemType> *tempPtr = this->m_head->m_next;
        tempPtr != NULL;
        tempPtr = tempPtr->m_next, this->m_curPosition++)
    {
        if(tempPtr->m_data == elem)
        {
...
//操作结果：获取第position个结点的数据
template <class ElemType>
ElemType LinkList<ElemType>::GetElem(int position)                        // 获取第position个结点的数据
{
    int length = this->Length( );
    ...
    else        // 否则如果position合法
    {
       ...
       // system("pause");
        //std::cout <<tempPtr;
      //  system("pause");
        return tempPtr->m_data;            // 用e返回第podition个元素的值
    }
...
//操作结果：删除链表第position个结点的数据, 并将此数据返回给elem
template <class ElemType>
ElemType LinkList<ElemType>::Delete(int position)         // 删除链表第position个结点的数据, 并将此数据返回给elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Delete..." <<std::endl;
  ...
  //  {
        ...
        Node<ElemType> *nextPtr = tempPtr->m_next;  // 凭借找到的结点找到要删除的位置
        tempPtr->m_next = tempPtr->m_next->m_next;//nextPtr->m_next;          // 重新建立删除后的联系
        ElemType elem = nextPtr->m_data;                     // 用elem返删结点的数据域
