...
*  File Name  : main.cpp                                                     *
*  Description: 邻接矩阵无向图类AdjListUndirGraph主函数源文件                               *
*  Author     : Gatie_Me                                                     *
...
// 内部函数(main.cpp)
// 建立一个邻接矩阵无向图
void CreateGraph(AdjListUndirGraph<char> &graph);
// 修改图的信息
void InsertGraph(AdjListUndirGraph<char> &graph);
void DeleteGraph(AdjListUndirGraph<char> &graph);
// 遍历图的信息
void Travel(AdjListUndirGraph<char> &graph);


/// 主函数
int main( )
{
    system("title 邻接表无向图");     // 调用系统命令更改DOS窗口名称
...
void CreateGraph(AdjListUndirGraph<char> &graph)
{
    ...
    vertexs = new char[vertexNum];  // 开辟结点信息    for(i = 0; i < n; i++)
    for(i = 0; i < vertexNum; i++)
    {
        cin >>vertexs[i];
    }

    graph.SetGraph(vertexs, vertexNum);
  //  system("pause");
    i = 0;
    while(i < vertexNum)
    {
        ...
        if(graph.InsertEdge(i, j) == true)
        {
            cout <<"插入边<" <<graph.GetVertexElem(i) <<", "<<graph.GetVertexElem(j) <<">" <<std::endl;
        }
    }
