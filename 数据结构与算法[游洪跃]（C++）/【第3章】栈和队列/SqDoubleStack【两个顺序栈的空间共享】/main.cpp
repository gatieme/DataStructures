...
int main( )
{
    ...
    int index;
    SqDoubleStack<int> stack;     // 建立一个顺序栈
    ...
    while( 1 )
    {
        ...
        case 2:
            ...
            index = LeftOrRight( );
            while(stack.Empty(index) == false)
            {
                cout <<"弹出的数据: "<<stack.Pop(index) <<endl;
            }
        ...
        case 4:
            ...
            index = LeftOrRight( );
            while(stack.Full() == false)
            {
                cout <<endl <<"请输入您要插入的数据[输入0结束插入]" <<endl;
                cin >>temp;
                if(temp == 0)
                {
                    break;
        ...
        case 5:
            ...
            {
                // 循环直到用户输入恰当的回答为止
                cout <<endl <<"请问您是否确定需要弹出栈顶数据" <<endl;
                cout << "(y, n)?" << "用y或n回答:";
                while ((answer = getchar( )) == '\n');	// 跳过空格,制表符及换行符获取一字符
                if(answer == 'y' || answer == 'Y')         // 只要栈不是空的
                {
                    if(stack.Pop(index, temp) == true)     // 弹出栈顶元素
                    {
                        cout <<"弹出栈顶数据" <<temp <<endl <<endl;
                    }
                }
                else if(answer != 'n' && answer != 'N')
                {
                    cout <<"信息输入有误, 请重新输入" <<endl <<endl;
                }

                if(stack.Empty(index) == true)   // 如果当前顺序栈为空, 则系统将自动返回主菜单
                {
                    break;
                }
            }
...
bool Match(char *str)
{
    SqDoubleStack<char> strStack;     // 字符串栈
    char tempCh;                // 临时字符
    ...
    for(int i = 0; str[i] != '\0'; i++)
    {
        ...
        {
            strStack.Push(-1, str[i]);    // 就将字符压入栈中
        }
        else if(str[i] == ')')  // 检查()是否匹配// 检查当前的()是否匹配
        {
            if(strStack.Empty(-1))    // 如果是个空栈, 则括号坑定不匹配
            {
                return false;
            }
            else if(strStack.Top(-1, tempCh), tempCh == '(')     // 如果当前这对括号匹配
            {
                strStack.Pop(-1, tempCh);           // 将已经匹配的括号弹出
            }
            else
            {
                return false;
