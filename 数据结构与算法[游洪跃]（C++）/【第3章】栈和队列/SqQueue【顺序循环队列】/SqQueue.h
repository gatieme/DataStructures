...
SqQueue<ElemType>::SqQueue(int size/* = DEFAULT_SIZE*/)         // 顺序循环队列构造函数模版
{
    ...
    this->m_maxSize = size;           // 设置队列的最大元素值
    this->m_front = this->m_rear = 0;   // 初始化当前队头和队尾都指向初始0下标
}
...
{
    this->m_elems = new ElemType[copy.m_maxSize]; // 依照赋值队列的大小信息开辟空间
    ...
    for(int curPosition = this->m_front;
            ...
            curPosition = (curPosition+1)%this->m_maxSize)
    {
...
std::cout <<"OutQueue..." <<std::endl;
    ...
    {
        elem = this->m_elems[m_front];
        this->m_front = (m_front + 1) % m_maxSize;
...
std::cout <<"InQueue..." <<std::endl;
    ...
    else
    {
        ...
        this->m_rear = (m_rear + 1) % m_maxSize;
...
std::cout <<"OutQueue..." <<std::endl;
#endif
    // 函数实现
    this->m_elems = new ElemType[copy.m_maxSize];
    if(m_elems == NULL)
    {
        #ifdef REMIND
        std::cout <<"系统内存不足" <<std::endl;
    ...
    for(int curPosition = m_front;
            curPosition != m_rear;
            curPosition = (curPosition + 1) % m_maxSize)
    {
