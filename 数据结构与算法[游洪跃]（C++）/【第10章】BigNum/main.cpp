...
{
    cout<<"************************************************" <<std::endl;
	cout<<"**               1 -=> 计算阶乘               **" <<std::endl;
	cout<<"**               2 -=> 计算次幂               **" <<std::endl;
	cout<<"**               0 -=> 结束游戏               **" <<std::endl;
...
void Pow( )
{
	...
	while(!flag)
	{
		...
		case 1:
            ...
            cout <<"请输入您的指数 exp = ";
            cin >>exp;
            cout <<BigNum(base) <<"的" <<BigNum(exp) <<"次幂" <<Power(BigNum(base), BigNum(exp)) <<endl;
            system("pause");
			break;
		case 2:
			flag = 1;							//退出循环
