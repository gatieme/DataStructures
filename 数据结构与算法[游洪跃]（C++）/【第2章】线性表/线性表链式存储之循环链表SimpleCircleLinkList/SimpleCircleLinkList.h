...
/*=========================【PUBLIC：公共接口】=========================*/
  public :
  // 类的基本成员函数
    SimpleCircleLinkList( );     // 默认构造函数模版

    SimpleCircleLinkList(const SimpleCircleLinkList<ElemType> &copy);   //赋值构造函数模版

    virtual ~SimpleCircleLinkList( );     // 虚析构函数模版
  ...
  // 重载函数模版
    SimpleCircleLinkList<ElemType> &operator=(const SimpleCircleLinkList<ElemType> &copy);  // 重载赋值运算符
...
template <class ElemType>
Node<ElemType>* SimpleCircleLinkList<ElemType>::GetElemPtr(int position) const // 返回指向第position个结点的指针
{
    // 调试信息
...
std::cout <<"GetElemPtr..." <<std::endl;
    ...
    // 否则的话寻找的是以后的接待你
    Node<ElemType> *tempPtr = this->m_head->m_next;  // 将数据
    int curPosition = 1;
...
//操作结果：初始化线性链表的数据信息
template <class ElemType>
void SimpleCircleLinkList<ElemType>::Init( )  // 初始化线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Init..." <<std::endl;
    ...
    this->m_head = new Node<ElemType>;      // 首先构造头指针
    this->m_head->m_next =  this->m_head;   // 设置循环链表为空
}
...
/**类的基本成员函数【实现】************************************************
    ...
    virtual ~SimpleCircleLinkList( );     // 虚析构函数模版
************************************************【实现】类的基本成员函数**/
//函数名称： SimpleCircleLinkList( );
//初始条件：线性表未建立
//操作结果：建立一个空链表
template <class ElemType>
SimpleCircleLinkList<ElemType>::SimpleCircleLinkList( )     // 默认构造函数模版
{
    // 当循环列表为空时, 头结点的指针域指向头结点
...
//函数名称： SimpleCircleLinkList(const SimpleCircleLinkList<ElemType> &copy);
//初始条件：线性表未建立
//操作结果：建立一个空链表
template <class ElemType>
...
//函数名称： virtual ~SimpleCircleLinkList( );     // 虚析构函数模版
//初始条件：线性表建立
//操作结果：销毁线性表
template <class ElemType>
SimpleCircleLinkList<ElemType>::~SimpleCircleLinkList( )        // 虚析构函数模版
{
    Clean( );
    delete this->m_head;
}
...
//操作结果：返回当前链表的表长
template <class ElemType>
int SimpleCircleLinkList<ElemType>::Length( ) const // 获取线性链表的长度
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::Empty( )const    // 判断线性链表是否为空
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
#endif
    // 函数实现
    return this->m_head->m_next == this->m_head;   // 如果头指针的下一个结点位置为仍是头结点, 则循环链表为空
}
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleCircleLinkList<ElemType>::Clean( )            // 清空当前线性链表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleCircleLinkList<ElemType>::Traverse(void(*visit)(const ElemType &)) const        //遍历线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::GetElem(int position, ElemType &elem)     //获取第position个结点的数据
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::SetElem(int position, const ElemType &elem)  // 设置第position个结点的数据域为elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Traverse..." <<std::endl;
...
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::Delete(int position, ElemType &elem)         // 删除链表第position个结点的数据, 并将此数据返回给elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::Insert(int position, const ElemType &elem)  // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleCircleLinkList<ElemType>::Inlist(const ElemType &elem)   // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Insert..." <<std::endl;
...
//    }       // 循环结束以后tempPt已经到了最后一个结点域
    ...
    Node<ElemType> *newPtr = new Node<ElemType>(elem, this->m_head);   // 新建一个结点域
...
/*************************************************************************************************
    SimpleCircleLinkList<ElemType> &operator=(const SimpleCircleLinkList<ElemType> &copy);  // 重载赋值运算符
**************************************************************************【实现】重载函数模版**/
//函数名称：SimpleCircleLinkList<ElemType> &operator=(const SimpleCircleLinkList<ElemType> &copy);
//初始条件：线性链表已经建立
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
SimpleCircleLinkList<ElemType>& SimpleCircleLinkList<ElemType>::operator=(const SimpleCircleLinkList<ElemType> &copy)     // 重载赋值运算符
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
/**特殊处理模版【实现】******************************************************
    ...
    bool SelectSort(bool minTomax = true);      //选择法排序
    bool QuickSort(int start, int end);         // 快速排序函数
    bool SimpleCircleLinkList<ElemType>::CutOddEven( )         // 快速排序函数
******************************************************【实现】特殊处理模版**/
//函数名称：void Swap( )
//初始条件：线性链表已经建立
//操作结果：交换线性链表第positionA个位置的元素和第positionB个位置的元素
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::Swap(int positionA, int positionB)                // 交换函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::BubbleSort(bool minTomax/*= true*/)     //冒泡法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::SelectSort(bool minTomax/*= true*/)      //选择法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：快速排序排序线性链表
template <class ElemType>
bool SimpleCircleLinkList<ElemType>::QuickSort(int start, int end)         // 快速排序函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
