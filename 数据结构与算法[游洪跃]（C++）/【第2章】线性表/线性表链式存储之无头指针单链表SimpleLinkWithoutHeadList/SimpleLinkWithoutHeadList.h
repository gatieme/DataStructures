...
版本1.0.2  :当前版本出现Bug, 删除和插入第一个元素的时候出错
********************************************************************************/
...
/*=========================【PUBLIC：公共接口】=========================*/
  public :
  // 类的基本成员函数
    SimpleLinkWithoutHeadList( );     // 默认构造函数模版

    SimpleLinkWithoutHeadList(const SimpleLinkWithoutHeadList<ElemType> &copy);   //赋值构造函数模版

    virtual ~SimpleLinkWithoutHeadList( );     // 虚析构函数模版
  ...
  // 重载函数模版
    SimpleLinkWithoutHeadList<ElemType> &operator=(const SimpleLinkWithoutHeadList<ElemType> &copy);  // 重载赋值运算符
...
template <class ElemType>
Node<ElemType>* SimpleLinkWithoutHeadList<ElemType>::GetElemPtr(int position) const // 返回指向第position个结点的指针
{
    // 调试信息
...
std::cout <<"GetElemPtr..." <<std::endl;
    ...
    // 函数实现
    if(this->m_first == NULL)      // 如果只有一个头结点
    {
        std::cout <<"当前线性链表为空, 无法查找第" <<position <<"个结点的信息" <<std::endl;
        return NULL;            // 返回空
    }

    Node<ElemType> *tempPtr = this->m_first;     // 用指针tempPtr遍历线性链表查找第position个结点
...
//操作结果：初始化线性链表的数据信息
template <class ElemType>
void SimpleLinkWithoutHeadList<ElemType>::Init( )  // 初始化线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
/**类的基本成员函数【实现】************************************************
    ...
    virtual ~SimpleLinkWithoutHeadList( );     // 虚析构函数模版
************************************************【实现】类的基本成员函数**/
//函数名称： SimpleLinkWithoutHeadList( );
//初始条件：线性表未建立
//操作结果：建立一个空链表
template <class ElemType>
SimpleLinkWithoutHeadList<ElemType>::SimpleLinkWithoutHeadList( )     // 默认构造函数模版
{
    this->m_first = new Node<ElemType>;        // 构造头指针
}

//函数名称： SimpleLinkWithoutHeadList(const SimpleLinkWithoutHeadList<ElemType> &copy);
//初始条件：线性表未建立
//操作结果：建立一个空链表
template <class ElemType>
...
//    Node<ElemType> *copyPtr = copy.m_first;    // 建立一个结点指向初始化对象的第一个信息
//    Node<ElemType> *tempPtr;                          // 新建一个结点, 作为循环结点域
//    tempPtr = this->m_first;                   // 将当前结点域与头指针建立联系
//
//    while(tempPtr->m_next != NULL)   // 控制循环
//    {
//        // 首先建立一个结点存放下一个数据　
...
//函数名称： virtual ~SimpleLinkWithoutHeadList( );     // 虚析构函数模版
//初始条件：线性表建立
//操作结果：销毁线性表
template <class ElemType>
SimpleLinkWithoutHeadList<ElemType>::~SimpleLinkWithoutHeadList( )        // 虚析构函数模版
{
    Clean( );
    delete this->m_first;
}
...
//操作结果：返回当前链表的表长
template <class ElemType>
int SimpleLinkWithoutHeadList<ElemType>::Length( ) const // 获取线性链表的长度
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::Empty( )const    // 判断线性链表是否为空
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
#endif
    // 函数实现
    return this->m_first == NULL;   // 如果头指针的下一个结点位置为空, 则此线性表为空
}
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleLinkWithoutHeadList<ElemType>::Clean( )            // 清空当前线性链表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Clean..." <<std::endl;
    ...
    //while(this->Length() > 0)
    while(this->m_first != NULL)   // 只要线性链表不为空
    {
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleLinkWithoutHeadList<ElemType>::Traverse(void(*visit)(const ElemType &)) const        //遍历线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::GetElem(int position, ElemType &elem)     //获取第position个结点的数据
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::SetElem(int position, const ElemType &elem)  // 设置第position个结点的数据域为elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Traverse..." <<std::endl;
...
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::Delete(int position, ElemType &elem)         // 删除链表第position个结点的数据, 并将此数据返回给elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Delete..." <<std::endl;
    ...
    else            // 否则索引position合法
    {
        ...
        else if(position == 1)
        {
            ...
            this->m_first = this->m_first->m_next;            // 删除头指针后那么后一个结点就是头指针了
            delete tempPtr;     // 删除原来头指针的空间
        }
...
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::Insert(int position, const ElemType &elem)  // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Insert..." <<std::endl;
    ...
    else            // 否则索引position合法
    {
        if(position > 1)
        {
            Node<ElemType> *tempPtr = this->GetElemPtr(position-1);   // 找到要插入位置的前一个结点域
            Node<ElemType> *newPtr = new Node<ElemType>(elem, tempPtr->m_next);  // 将新的结点与他插入位置的下一个结点建立联系
            tempPtr->m_next = newPtr;
        }
        else//if(psotion == 1)
        {   /// 代码有问题
            Node<ElemType> *newPtr = new Node<ElemType>(elem, this->m_first);      // 建立一个头指针, 作为新的头指针
            this->m_first = newPtr;
        }
        return true;
    }
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void SimpleLinkWithoutHeadList<ElemType>::Inlist(const ElemType &elem)   // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Insert..." <<std::endl;
#endif
    if(this->m_first == NULL)  // 如果头结点为空, 说明该线性链表是空的
    {
        this->m_first->SetNode(elem);     // 直接更改头指针的数据域
    }
    else
    {
        Node<ElemType> *tempPtr = this->m_first;
...
/*************************************************************************************************
    SimpleLinkWithoutHeadList<ElemType> &operator=(const SimpleLinkWithoutHeadList<ElemType> &copy);  // 重载赋值运算符
**************************************************************************【实现】重载函数模版**/
//函数名称：SimpleLinkWithoutHeadList<ElemType> &operator=(const SimpleLinkWithoutHeadList<ElemType> &copy);
//初始条件：线性链表已经建立
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
SimpleLinkWithoutHeadList<ElemType>& SimpleLinkWithoutHeadList<ElemType>::operator=(const SimpleLinkWithoutHeadList<ElemType> &copy)     // 重载赋值运算符
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//        Node<ElemType> *copyPtr = copy.m_firstt;    // 建立一个结点指向初始化对象的第一个信息
...
/**特殊处理模版【实现】******************************************************
    ...
    bool SelectSort(bool minTomax = true);      //选择法排序
    bool QuickSort(int start, int end);         // 快速排序函数
    bool SimpleLinkWithoutHeadList<ElemType>::CutOddEven( )         // 快速排序函数
******************************************************【实现】特殊处理模版**/
//函数名称：void Swap( )
//初始条件：线性链表已经建立
//操作结果：交换线性链表第positionA个位置的元素和第positionB个位置的元素
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::Swap(int positionA, int positionB)                // 交换函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::BubbleSort(bool minTomax/*= true*/)     //冒泡法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::SelectSort(bool minTomax/*= true*/)      //选择法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：快速排序排序线性链表
template <class ElemType>
bool SimpleLinkWithoutHeadList<ElemType>::QuickSort(int start, int end)         // 快速排序函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
