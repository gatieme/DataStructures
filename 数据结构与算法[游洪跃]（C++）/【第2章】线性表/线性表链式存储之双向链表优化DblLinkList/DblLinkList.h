...
/*=========================【PUBLIC：公共接口】=========================*/
  public :
  // 类的基本成员函数
    DblLinkList( );     // 默认构造函数模版

    DblLinkList(const DblLinkList<ElemType> &copy);   //赋值构造函数模版

    virtual ~DblLinkList( );     // 虚析构函数模版
...
/*========================【PROTECTED：保护成员】========================*/
  ...
  // 保护数据成员
    ...
    mutable DblNode<ElemType> *m_curPtr;   // 指向当前位置的指针
...
template <class ElemType>
DblNode<ElemType>* DblLinkList<ElemType>::GetElemPtr(int position) const // 返回指向第position个结点的指针
{
    // 调试信息
...
std::cout <<"GetElemPtr..." <<std::endl;
    ...
    {   // 如果当前访问位置在所查找位置以前, 则向后查找
        for( ; this->m_curPosition < position; this->m_curPosition++)
        {
            this->m_curPtr =  this->m_curPtr->m_next;     // 结点依次往后找
        }

    }
    else if(this->m_curPosition > position)
    {   // 如果当前访问位置在所查找位置以后, 则需要往前查找
        for( ; this->m_curPosition > position; this->m_curPosition--)
        {
            this->m_curPtr = this->m_curPtr->m_back;    // 结点依次往前找
        }
    }

    return this->m_curPtr;      // 放回访问信息
}
...
//操作结果：初始化线性链表的数据信息
template <class ElemType>
void DblLinkList<ElemType>::Init( )  // 初始化线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Init..." <<std::endl;
    ...
    // 初始化访问信息
    this->m_curPosition = this->m_head;     // 初始元素访问位置为头指针
    this->m_curPtr = 0;                     // 初始化元素访问位置下标为0
    this-m_length = 0;                      // 元素个数初始化为0
}
...
//操作结果：建立一个空链表
template <class ElemType>
DblLinkList<ElemType>::DblLinkList( )     // 默认构造函数模版
{
    // 当循环列表为空时, 头结点的指针域指向头结点
    ...
    this->m_head->m_back = this->m_head;
    // 初始化访问信息
    this->m_curPosition = 0;     // 初始元素访问位置为头指针
    this->m_curPtr = this->m_head;                     // 初始化元素访问位置下标为0
    this->m_length = 0;                      // 元素个数初始化为0
}

//函数名称： DblLinkList(const DblLinkList<ElemType> &copy);
//初始条件：线性表未建立
//操作结果：建立一个空链表
template <class ElemType>
...
//函数名称： virtual ~DblLinkList( );     // 虚析构函数模版
//初始条件：线性表建立
//操作结果：销毁线性表
template <class ElemType>
DblLinkList<ElemType>::~DblLinkList( )        // 虚析构函数模版
{
    Clean( );
    delete this->m_head;
}
...
//操作结果：返回当前链表的表长
template <class ElemType>
int DblLinkList<ElemType>::Length( ) const // 获取线性链表的长度
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool DblLinkList<ElemType>::Empty( )const    // 判断线性链表是否为空
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
#endif
    // 函数实现
    return (this->m_length == 0);   // 如果头指针的下一个结点位置为仍是头结点, 则循环链表为空
}
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void DblLinkList<ElemType>::Clean( )            // 清空当前线性链表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void DblLinkList<ElemType>::Traverse(void(*visit)(const ElemType &)) const        //遍历线性表
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool DblLinkList<ElemType>::GetElem(int position, ElemType &elem)     //获取第position个结点的数据
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool DblLinkList<ElemType>::SetElem(int position, const ElemType &elem)  // 设置第position个结点的数据域为elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Traverse..." <<std::endl;
...
template <class ElemType>
bool DblLinkList<ElemType>::Delete(int position, ElemType &elem)         // 删除链表第position个结点的数据, 并将此数据返回给elem
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
template <class ElemType>
bool DblLinkList<ElemType>::Insert(int position, const ElemType &elem)  // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
void DblLinkList<ElemType>::Inlist(const ElemType &elem)   // 将数据elem插入到线性链表的末尾
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
std::cout <<"Insert..." <<std::endl;
...
/*************************************************************************************************
    DblLinkList<ElemType> &operator=(const DblLinkList<ElemType> &copy);  // 重载赋值运算符
**************************************************************************【实现】重载函数模版**/
//函数名称：DblLinkList<ElemType> &operator=(const DblLinkList<ElemType> &copy);
//初始条件：线性链表已经建立
//操作结果：如果当前线性链表为空, 返回True, 否则返回False
template <class ElemType>
DblLinkList<ElemType>& DblLinkList<ElemType>::operator=(const DblLinkList<ElemType> &copy)     // 重载赋值运算符
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
/**特殊处理模版【实现】******************************************************
    ...
    bool SelectSort(bool minTomax = true);      //选择法排序
    bool QuickSort(int start, int end);         // 快速排序函数
    bool DblLinkList<ElemType>::CutOddEven( )         // 快速排序函数
******************************************************【实现】特殊处理模版**/
//函数名称：void Swap( )
//初始条件：线性链表已经建立
//操作结果：交换线性链表第positionA个位置的元素和第positionB个位置的元素
template <class ElemType>
bool DblLinkList<ElemType>::Swap(int positionA, int positionB)                // 交换函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool DblLinkList<ElemType>::BubbleSort(bool minTomax/*= true*/)     //冒泡法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：马泡泡发排序线性链表
template <class ElemType>
bool DblLinkList<ElemType>::SelectSort(bool minTomax/*= true*/)      //选择法排序
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
...
//操作结果：快速排序排序线性链表
template <class ElemType>
bool DblLinkList<ElemType>::QuickSort(int start, int end)         // 快速排序函数
{
    // 调试信息
#if defined DEBUG           // 如果宏定义了DEBUG, 执行一下语句
